...
// target - Active source, direct delivery mode

// NOTE: As the sourcecfgK.D is invisible in S-domain, the effective value of the non-delegated source configuration is read-only zero.

==== Interrupt-Enable and Interrupt-Pending Control Registers

// TODO: drop offset, access. Add note that these are existing registers in APLIC
...
|  setipnum      |  RW     |  Set interrupt-pending by number
|  clripnum      |  RW     |  Clear interrupt-pending by number
|  setienum      |  RW     |  Set interrupt-enable by number
|  clrienum      |  RW     |  Clear interrupt-enable by number
|===
...
|===
|  Register name |  Access |  Description
|  idelivery     |  RW     |  External interrupt delivery enable
|  ithreshold    |  RW     |  External interrupt threshold
|  topi          |  RO     |  Top priority external interrupt information
|  claimi        |  RW     |  Top priority external interrupt claim
|===
